...
app = FastAPI(title="Opal-Central", root_path=ROOT_PATH)
...
def email_user(
    ...
    lines = [
        f"{session['username']} flagged this customer in Opal-Central:",
        "",
    ...
    lines += ["", f"View the full record: {record_url}", "", "— Opal-Central"]
    body = "\n".join(lines)
...
async def admin_upload(request: Request, file: UploadFile = File(...)):
    ...
    for c in new_critical:
        ...
        body = (
            ...
            f"View in Opal-Central: {get_setting('app_url', 'http://localhost:8443')}\n"
        )
...
async def db_maintenance_merge(request: Request):
    ...
    # Survivor = hottest temperature (lowest order), tie-break by earliest id
    survivor = min(records, key=lambda r: (r["temperature_order"], r["id"]))
    others = [r for r in records if r["id"] != survivor["id"]]

    # Concatenate text fields — survivor's value first, then others, skip blanks
    updates = {}
    ...
    # Reassign heat_history from deleted records to survivor, then delete them
    merged_names = []
    for r in others:
